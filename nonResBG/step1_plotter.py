...
#indir="./nonResSkim" # only MuEG gives difference wrt 20160915 results @20160916
indir="./nonResSkim_Lite_20160915"
...
# scaleElMu switched off because mZ will be scaled to give numbers in this code:
plotter_eu=InitializePlotter(indir, addSig=False, addData=True,doRatio=True, doElMu=True, scaleElMu=False, LogY=logy)
stack_ll = plotter_ll.GetStack()
stack_eu = plotter_eu.GetStack()
...
### ----- Execute (plotting):
stackTagger=[tag, 'met'+met_cut]
stackTag='_'.join(stackTagger)
#Inclusive stack plot:
stack_ll.drawStack('llnunu_l1_mass', cuts['ll']['inclusive'], str(lumi*1000), 13, 50.0, 180.0, titlex = "M_{Z}^{ll}", units = "GeV",
                   output=stackTag+'_mll',outDir=outdir, separateSignal=True,
                   drawtex="", channel="")
stack_eu.drawStack('elmununu_l1_mass', cuts['emu']['inclusive'], str(lumi*1000), 13, 50.0, 180.0, titlex = "M_{Z}^{e#mu}", units = "GeV",
                   output=stackTag+'_melmu',outDir=outdir, separateSignal=True,
                   drawtex="", channel="")
...
sfFile=ROOT.TFile(outdir+'/'+"shape_correction"+"_met"+met_cut+".root","recreate")
sfFile.cd()
h_ll_shape.Write()
