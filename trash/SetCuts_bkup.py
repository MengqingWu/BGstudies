...
class SetCuts ():
    ...
    def alphaCuts(self, isIn=True, isll=True, inclusive=False):
        astr = "({0}&&{1}&&{2}&&{3}&&{4})"
        cuts = astr.format(*self.cutflow)

        cuts_tmp= ROOT.TString(cuts)
        
        if inclusive: cuts_tmp.ReplaceAll("abs(llnunu_l1_mass-91.1876)<20.0&&","")
        elif not isIn: cuts_tmp.ReplaceAll("91.1876)<20.0","91.1876)<=55.0&&abs(llnunu_l1_mass-91.1876)>=25.0")
        else: pass
